"""
Manipulation de mots à partir d’un corpus.
"""
...
def mots_de_n_lettres(mots, n):
    ...
    """
    ensemble = {mot for mot in mots if len(mot)==n}
    return ensemble
...
def mots_avec(mots, s):
    ...
    """
    ensemble = {mot for mot in mots if s in mot}
    return ensemble
...
def cherche1(mots, start, stop, n):
    ...
    """
    ensemble = {mot for mot in mots if len(mot)==n and mot.startswith(start) and mot.endswith(stop)}
    return ensemble
...
def cherche2(mots, lstart, lmid, lstop, nmin, nmax):
    ...
    for mot in mots:
        ...
        if not any(mot.startswith(p) for p in lstart):
            continue

        if not any(m in mot for m in lmid):
            continue

        if not any(mot.endswith(s) for s in lstop):
            continue
...
def main():
    ...
    Elle charge le corpus et teste les différentes fonctions
